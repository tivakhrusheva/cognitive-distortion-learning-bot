        ...
        state: Question
            script:
                if ($session.thought && $session.emotion && $session.authothought) {
                    $reactions.answer("Пожалуйста, опишите ситуацию и ваши негативные мысли по ее поводу, или нажмите на кнопку 'Использовать прошлый запрос', чтобы в качестве запроса был отправлен ваш последний запрос из дневника автоматических мыслей и искажений:\n\n<b>Мысль</b>: {{$session.thought}}.\n<b>Я чувствую</b> {{$session.emotion}}.\n<b>Я думаю, что</b> {{$session.autothought}}.\n\n⬇️Чтобы посмотреть примеры ситуаций, намжите на кнопку 'Посмотреть примеры ситуаций'.");
